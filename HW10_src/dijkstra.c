...
// dijkstra_set_graph - implementace funkce do HW10.
_Bool dijkstra_set_graph(int e, int edges[][3], void* dijkstra) {
     ...
     if (
        ...
        if(dij->graph)
          free(dij->graph);
        dij->graph = malloc(sizeof(graph_t));
        if(dij->graph == NULL)
          return false;
        int m = -1;
        for (int i = 0; i < dij->graph->num_edges; ++i) {
...
// dijkstra_get_solution - implementace funkce do HW10.
_Bool dijkstra_get_solution(const void* dijkstra, int n, int solution[][3]) {
     const dijkstra_t* const d = (const dijkstra_t*)dijkstra;
     if(d == NULL || d->num_nodes > n)
          return false;
